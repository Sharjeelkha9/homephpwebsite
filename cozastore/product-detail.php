...
if(isset($_GET['pid'])){
	$pid = $_GET['pid'];
	$query = $pdo ->prepare("SELECT `products`.*, `categories`.`name` as ctname
FROM `products` 
	inner JOIN `categories` ON `products`.`categoryid` = `categories`.`ctid` where id = :pid");
...
?>
	...
	<div class="container m-t-60">
		<div class="bread-crumb flex-w p-l-25 p-r-15 p-t-30 p-lr-0-lg">
			...
			<a href="product.php?cid=<?php echo $row['categoryid']?>" class="stext-109 cl8 hov-cl1 trans-04">
			<?php
			echo $row['ctname'];
			?>
				<i class="fa fa-angle-right m-l-9 m-r-10" aria-hidden="true"></i>
